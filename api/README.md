...
```bash
sam-lam$ sam build --use-container
```
...
```bash
sam-lam$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-lam$ sam local start-api
sam-lam$ curl http://localhost:3000/
```
...
```bash
sam-lam$ sam logs -n HelloWorldFunction --stack-name sam-lam --tail
```
...
```bash
sam-lam$ pip install -r tests/requirements.txt --user
# unit test
sam-lam$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-lam$ AWS_SAM_STACK_NAME="sam-lam" python -m pytest tests/integration -v
```
